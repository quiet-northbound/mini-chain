...
**Project Name**: Build-a-Mini-Layer2
...
│  • Áp dụng tx hoặc verify L2 commit             │
...
│  • Gom batch                                     │
│  • Tạo commitment:                               │
│    batchId, txCount, preStateRoot,               │
│    postStateRoot, metadata                       │
...
|------|-----------|
| Ngôn ngữ | **C# / .NET 8** |
| Runtime | Local, single node |
| Storage | In-memory (có thể ghi file nếu cần) |
| Hash | SHA-256 |
...
3. Mỗi bước phải giải thích lại được cho người khác
4. Nếu mơ hồ → **dừng lại**, làm rõ khái niệm trước khi code
...
|-----------|--------|--------|
| **M0** | Ledger + accounts + signed transaction (mock signature) | ✅ Done |
| **M1** | Layer 1 mini-chain + apply tx + validate nonce/balance | ✅ Done |
...
> 📌 **Ghi chú**: Project này ưu tiên hiểu sâu và build từng phần chạy được, không chạy theo độ phức tạp hay chuẩn production.
